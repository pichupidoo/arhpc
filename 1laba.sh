...
DEFAULT_INTERFACE=$(ip route show to default | awk '{print $5}')  #route show to default — выводит маршрут по умолчанию, то есть информацию о сетевом интерфейсе, который используется для доступа в интернет или к внешним сетям. Маршрут по умолчанию указывает, через какой интерфейс передаётся трафик, если не указаны другие маршруты.
